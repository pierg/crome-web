...
import "./ContractAccordionItem"
import "./ContractModalDetails"
import "react"
import "reactstrap"
import Checkbox
import ContractAccordionItem
...
function GoalView(props) {
  ...
  for (const property in props.contract) {
    if (props.contract.hasOwnProperty(property)) {
      ...
      contract[Object.keys(props.contract).indexOf(property)]["content"] =
  ...
  function parseContext(context) {
    return context === undefined
      ? ["", ""]
      : [
          context.includes("day") ? "checked" : "",
          context.includes("night") ? "checked" : "",
        ];
  }
  ...
  return (
    <>
      <div
        className={
          "w-full lg:w-6/12 xl:w-5/12 mt-8 ml-4 mr-4 px-4 relative flex flex-col min-w-0 break-words bg-white rounded mb-6 xl:mb-0 shadow-lg opacity-1 transform duration-300 transition-all ease-in-out"
        }
      >
        <div className="flex-auto p-4 pr-0">
            ...
            <div className="flex flex-wrap w-full">
              ...
              <div className="flex flex-col flex-wrap w-full">
                <div className="relative w-full pr-4 mt-2 mb-2 max-w-full flex-grow flex-1">
                  <span className="text-md text-blueGray-700">{props.description}</span>
                </div>
                <div className="relative w-full pr-4 max-w-full flex-grow flex-1">
                  <Checkbox
                    className="mr-4"
                    label="Day"
                    readOnly
                    checked={parseContext(props.context)[0]}
                  />
                  <Checkbox
                    label="Night"
                    readOnly
                    checked={parseContext(props.context)[1]}
                  />
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col mt-4">
            <div className="relative w-full max-w-full flex-grow flex-1">
              <div
                className={
                  "overflow-hidden relative flex flex-col min-w-0 " +
                  (open ? "pb-4 " : "") +
                  "break-words bg-white w-full border-b border-blueGray-200"
                }
              >
