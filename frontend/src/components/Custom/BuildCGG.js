...
function BuildCGG(props) {
  ...
  function changeOperator(operator) {
    if (operator === "Refinement" || props.selectedOperator === "Refinement") {
      operator === "Refinement"
        ? setInfoMessage(props.infos.refinementMessages[0])
        : setInfoMessage("");
    ...
    if (props.selectedGoals.includes(goal)) {
      ...
      if (props.selectedOperator === "Refinement") {
        switch (tmpSelectedGoals.length) {
          case 0:
            setInfoMessage(props.infos.refinementMessages[1]);
            break;
          case 1:
            setInfoMessage(
              getGoalName(props.selectedGoals[0]) + " --> " + getGoalName(goal)
            );
            break;
          case 2:
            setInfoMessage(props.infos.refinementMessages[2]);
            return;
          default:
            break;
        }
  ...
  function getGoalName(node) {
    if (node.hasOwnProperty("name")) {
      ...
      return typeof node === "object" && node.hasOwnProperty("id")
        ? props.findGoalById(node.id)["name"]
        : props.findGoalById(node)["name"];
    }
  }
      ...
      <div className="w-full flex justify-center my-4">
        {props.infos.operators.map((prop, key) => (
          <Radio
            key={key}
            label={prop}
            onChange={() => changeOperator(prop)}
            checked={props.selectedOperator === prop}
            name="operator"
          />
        ))}
      </div>
      <div className="flex w-full justify-center">{infoMessage}</div>
      <div className="flex">
          ...
          <div className="flex flex-col">
            {props.goals !== null &&
              props.goals.map((prop, key) => (
                <Checkbox
                  key={key}
                  onChange={() => changeGoals(prop.id)}
                  checked={props.selectedGoals.includes(prop.id) ? "checked" : ""}
                  label={prop.name}
                />
              ))}
          </div>
