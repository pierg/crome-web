...
import "../../_texts/admin/footers/footeradmin"
import "../../components/Crome/IndexEnvironment"
import "../../components/Crome/WorldEdit"
import "../../components/Custom/CustomDownload"
import "../../components/Custom/ListBlock"
import "../../components/Custom/ListLine"
import "../../components/Custom/Location"
import "../../components/Custom/LocationIdEdit"
import "../../components/Custom/SavingEdit"
import "../../components/Elements/Button"
import "../../components/Footers/Admin/FooterAdmin"
import "../../hooks/arrayEquals"
import "../../hooks/deleteSubArrays"
...
export default class CreateEnvironment extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    this.listOfNames = [];
    if (this.props.world !== null) {
      this.listOfNames = this.getListWithoutElement(
        this.props.worldNames,
        ...
        this.setState(
          {
            mutexList: this.getMutexElements(
              this.state.editedLists[listIndex][elementIndex]
            ),
          },
  ...
  buildGrid(canvas, size, map, addLocation, callbackMap, updateErrorMsg, setNode) {
    ...
    world.onclick = function (node) {
      ...
      if (node.x % 2 === 1 && node.y % 2 === 1) {
        // if the user click on a cell
        if (start.length !== 0) {
          // when it's the second click, open the modal to get the Id entered by the user
          world.setClickedNode(node.x, node.y);
          setNode(node);
        } else {
          // when it's the first click, color the cell with a "light" color so the user know that he needs to click on a other cell
        ...
        (node.x % 2 === 0 && node.y % 2 === 1) ||
        (node.x % 2 === 1 && node.y % 2 === 0)
      ) {
        // when you click on wall cell
        if (startWall.length !== 0) {
          // when it's the second click
          endWall.push(node.x);
          endWall.push(node.y);
          let min;
          let max;

          if (startWall[0] === endWall[0] && startWall[1] === endWall[1]) {
            // when you double-click on a wall
            if (previousColorWall === "black") {
              // if the wall was black the background color will be white
              world.setColorIdBlocked(startWall[0], startWall[1], "white", false, null);
              world.reset();
              world.updateMap(map);
            } else if (previousColorWall === "white" || previousColorWall === null) {
              // if the wall was white the background color will be black
              world.setColorIdBlocked(startWall[0], startWall[1], "black", true, null);
              world.reset();
              world.updateMap(map);
            }
          } else if (startWall[0] === endWall[0]) {
            // when the users select a column
            if (startWall[0] % 2 === 1) {
              // when he clicks on a case that he can't choose
              ...
              for (let i = min; i < max + 1; i += 1) {
                if (
                  world.isBlocked(startWall[0], i) &&
                  world.getBackgroundColor(startWall[0], i) !== "black" &&
                  previousColorWall !== "black"
                ) {
                  // if there is a block of colour in the selected column
            ...
            if (startWall[1] % 2 === 1) {
              ...
              for (let i = min; i < max + 1; i += 1) {
                if (
                  world.isBlocked(i, startWall[1]) &&
                  world.getBackgroundColor(i, startWall[1]) !== "black" &&
                  previousColorWall !== "black"
                ) {
                  // if there is a block of colour in the selected column
                  updateErrorMsg(createenvironment.errorMsg.wallInsideBlock);
          ...
          world.setPreviousColorWall(
            world.getBackgroundColor(startWall[0], startWall[1])
          );
          if (
            world.getBackgroundColor(node.x, node.y) === "white" ||
            world.getBackgroundColor(node.x, node.y) === "black"
          ) {
            world.setBackgroundColor(node.x, node.y, "lightgray");
          } else {
            // when he clicks on a case that he can't choose
  ...
  drawLocation = () => {
    ...
    const answer = this.world.askToColor(
      this.state.currentIdInput,
      limits.minX,
      limits.maxX,
      limits.maxY,
      limits.minY,
    ...
    for (let i = 0; i < this.map.length; i++) {
      for (let j = 0; j < this.map[0].length; j++) {
        if (
          ...
          obj.grid.locations[index].coordinates.push({
            x: Math.trunc(i / 2) + 1,
            y: Math.trunc(j / 2) + 1,
          });
  ...
  isAdjacent(coord_a, coord_b) {
    for (let i = 0; i < coord_a.length; i++) {
      for (let j = 0; j < coord_b.length; j++) {
        if (
          Math.abs(coord_a[i]["x"] - coord_b[j]["x"]) +
            Math.abs(coord_a[i]["y"] - coord_b[j]["y"]) ===
          1
        ) {
          return true;
        }
    ...
    return (
      <>
                ...
                <div id="body" className="flex justify-center items-center">
                  <div className="flex container px-4 justify-center">
                      ...
                      <div className="flex flex-col items-center">
                        <div className="w-full ml-4">
                          <Link to="/index" className="hover-no-underline">
                            ...
                            >
                              <i
                                className={
                                  createenvironment.buttons.back.icon + " mr-2"
                                }
                              />
                        ...
                        <div className="m-4 px-16 pt-2 pb-2 relative flex flex-col min-w-0 break-words bg-white rounded shadow-lg">
                          ...
                          <div className="flex pl-2">
                            <Button color="red" onClick={() => this.modifyGridSize(-1)}>
                              <i className="text-xl fas fa-minus-square" />
                            </Button>
                            <Button
                              color="lightBlue"
                    ...
                    <div className="w-full lg:w-4/12 xl:w-3/12 flex-col mt-4">
                      <div
                        id="tooltipHelpBuild"
                        className="m-4 px-4 relative flex flex-col min-w-0 break-words bg-white rounded shadow-lg"
                      >
        ...
        >
          <WorldEdit
            element={
              this.state.editedLists[this.state.currentList][this.state.currentIndex]
            }
